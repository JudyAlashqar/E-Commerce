...
namespace WebProject.Controllers
{
    ...
    public class AttributeController : BaseController
    {
        ...
        public IActionResult Add(AttributeViewModel avm)
        {
            ...
            {
                var config = new MapperConfiguration(cfg => cfg.CreateMap<AttributeViewModel, AppDbContext.Models.Attribute>());
                var mapper = new Mapper(config);
                AppDbContext.Models.Attribute a = mapper.Map<AppDbContext.Models.Attribute>(avm);
        ...
        {
            AppDbContext.Models.Attribute attribute = _uow.attributeRepo.getById(id);
            if (attribute == null)
            {
                return RedirectToAction("Index");
            }
            ViewBag.id = id;
            AttributeViewModel avm = new AttributeViewModel();
            var config = new MapperConfiguration(cfg => cfg.CreateMap<AppDbContext.Models.Attribute, AttributeViewModel>());
            var mapper = new Mapper(config);
        ...
        public IActionResult Edit(AttributeViewModel avm)
        {
            if (ModelState.IsValid)
            {
                AppDbContext.Models.Attribute attribute= _uow.attributeRepo.getById(Convert.ToInt32(Request.Form["id"]));
                var config = new MapperConfiguration(cfg => cfg.CreateMap<AttributeViewModel, AppDbContext.Models.Attribute>());
                var mapper = new Mapper(config);
